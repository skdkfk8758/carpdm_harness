...
- **대규모 변경 감지**: 변경 파일 5개 이상 또는 새 모듈/도메인 생성 시, `docs/templates/sdd-template.md` 기반의 상세 설계서(SDD) 작성을 권장한다. SDD는 인터페이스 계약, 데이터 모델, 상태 흐름, 예외처리를 포함하여 plan.md보다 상세한 앵커 역할을 한다.
